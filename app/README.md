...
app 用于启动、展示和验证基础模块。当前只显示 Compose 欢迎页面，尚未实现登录、网络列表页或 MVI。

应用 ID / 命名空间为 `com.ttxz.base`，minSdk 26；依赖 common 和 network，并配置 Compose、Hilt、KSP。

| 文件 | 作用 |
| --- | --- |
| [BaseApp.kt](src/main/java/com/ttxz/base/BaseApp.kt) | 带 `@HiltAndroidApp` 的 Application，Hilt 宿主入口 |
| [MainActivity.kt](src/main/java/com/ttxz/base/MainActivity.kt) | `@AndroidEntryPoint` Activity，通过 setContent 展示主题和 Greeting |
...
Android Studio Sync 后选择 app，在 API 26 及以上设备运行。当前欢迎文字为 `Hello AndroidMVIBase!`，页面没有网络请求。
...
4. 以后再学习 ViewModel、StateFlow 和生命周期收集；当前没有这条状态链路，不能从欢迎页推断它已经实现。
